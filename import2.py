...
def exec(ab: str, tex_fmt: TextureFormat):
        ...
        for x in exts:
            ...
            if os.path.exists(src):
                ...
                if sprite is not None:
                    ...
                    match = re.search(r'\d+', sprite.m_Name)
                    if match:
                        number = int(match.group())+increment
                        sprite.m_Name=f"bg_{number}"
                    sprite.save()
                tex2d.m_Width, tex2d.m_Height = img.size
                tex2d.set_image(img, target_format=tex_fmt)
                match = re.search(r'\d+', tex2d.m_Name)
                if match:
                    number = int(match.group())+increment
                    tex2d.m_Name=f"bg_{number}"
                tex2d.save()
    ...
    if mod:
        path = os.path.join(outdir, tex2d.m_Name)
        with open(path, "wb") as f:
            f.write(env.file.save("original"))


increment = int(input("请输入bg_N开始序号：N="))
